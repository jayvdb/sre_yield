...
class YieldTest(unittest.TestCase):
    ...
    def testOverflowError(self):
        ...
        self.assertEqual(str(cm.exception), "cannot fit 'int' into an index-sized integer")
    ...
    def testGetItemNegativeIndexError(self):
        ...
        with self.assertRaises(IndexError) as cm:
            parsed.get_item(-len(parsed) - 1)
        self.assertEqual(str(cm.exception), "Index %d out of bounds" % (-len(parsed) - 1))
    ...
    def testUnsupportedErrors(self):
        ...
        with self.assertRaises(TypeError) as cm:
            parsed[:] = ["a"]
        self.assertEqual(str(cm.exception), "'RegexMembershipSequence' object does not support item assignment")

        with self.assertRaises(TypeError) as cm:
            del parsed[0]
        self.assertEqual(str(cm.exception), "'RegexMembershipSequence' object doesn't support item deletion")
